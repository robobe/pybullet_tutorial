...
class CartPoleBulletEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, discrete_actions=True):
        # start the bullet physics server
        self.render_mode = render_mode
        self._renders = render_mode == "human"
        ...
        if self._discrete_actions:
            ...
            self.action_space = spaces.Box(
                -action_high, action_high, shape=(action_dim,), dtype=np.float32
            )
...
class CartPoleContinuousBulletEnv(CartPoleBulletEnv):
    ...
    def __init__(self, render_mode=None):
        # start the bullet physics server
        CartPoleBulletEnv.__init__(
            self, render_mode=render_mode, discrete_actions=False
        )


def main():
    env = CartPoleBulletEnv(render_mode="human")
    no_of_episodes = 3
    max_steps = 500

    try:
        for i_episode in range(no_of_episodes):
            obs, info = env.reset(seed=i_episode)
            total_reward = 0.0
            ...
            for t in range(max_steps):
                ...
                if terminated or truncated:
                    print(
                        f"Episode {i_episode + 1}: steps={t + 1}, "
