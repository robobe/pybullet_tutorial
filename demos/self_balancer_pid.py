...
class SelfBalanceController:
    ...
    def update(self, current):
        ...
        xvel = -self.controller.update(setPoint, current)#calling the class in "pidcontrol.py" and getting the correction
...
def synthesizeData(robot):
    ...
    data = (p.getEulerFromQuaternion(p.getBasePositionAndOrientation(robot)[ORIENTATION_INDEX])[PITCH_INDEX])
    return data
...
if __name__ == "__main__":
    ...
    text_id = p.addUserDebugText("Pitch: 0.00", textPosition=[0, 0, 2], textColorRGB=[1, 0, 0])
    ...
    while(True):
        pitch=synthesizeData(robot)  #get robot state (pitch angle from IMU)
